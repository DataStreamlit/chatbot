...
# ── SCORE SHEET COLUMN NAMES ──────────────────────────────────────────────────────────────
# Matches SCORE_OUTPUT_COLS in or_aggregator.py (24 columns) — uses original column names
SCORE_COL_NAMES = [
    "Directorate",
    "Hospital Code",
    "Hospital Name",
...
# Alias used internally in TAB 10 for the emergency reconciled column
SCORE_EMERGENCY_COL = "Emergency Surgery Volume (Reconciled)"

# ── DATA LOADING ──────────────────────────────────────────────────────────────
def _load_score_sheet_from_excel(xf) -> pd.DataFrame:
    """
    Load the 'Score' sheet from an already-opened pd.ExcelFile object.
    Returns an empty DataFrame (with SCORE_COL_NAMES columns) if absent.
    """
    try:
        if "Score" not in xf.sheet_names:
            return pd.DataFrame(columns=SCORE_COL_NAMES)
        score_df = xf.parse("Score")
        if len(score_df.columns) == len(SCORE_COL_NAMES):
            score_df.columns = SCORE_COL_NAMES
        for c in SCORE_NUM_COLS:
...
def load_bytes(data):
    import io as _io
    xf = pd.ExcelFile(_io.BytesIO(data))

    df = xf.parse("Specialty Level Data")
    ...
    score_df = _load_score_sheet_from_excel(xf)
    return df, score_df
...
with st.sidebar:
    ...
    if bytes1 is not None:
        df1_raw, score1_raw = load_bytes(bytes(bytes1))
        label1 = name1
        st.success(f"✓ File 1 loaded — {len(df1_raw):,} rows")
    else:
        try:
            df1_raw, score1_raw = load_path("OR_Aggregated.xlsx")
            label1 = "Default aggregated file"
            st.info("File 1: using default file in same folder.")
        except Exception:
            try:
                df1_raw, score1_raw = load_path("Aggregated_Specialty_Level.xlsx")
                label1 = "Default aggregated file"
                st.info("File 1: using default file in same folder.")
            except Exception:
    ...
    if bytes2 is not None:
        df2_raw, score2_raw = load_bytes(bytes(bytes2))
        label2 = name2
...
# ══════════════════════════════════════════════════════════════════════════════
# TAB 10 — SCHEDULING INDICATOR
# Mirrors: مؤشر العمليات الفترة (OR Scheduling Indicator Report)
# ══════════════════════════════════════════════════════════════════════════════
with tab10:
    st.header("Scheduling Indicator — مؤشر العمليات")
    st.caption(
        "Mirrors the NHCC Scheduling Indicator report. "
        "**File 1** = this week · **File 2** = last week (for week-over-week comparison). "
        "Colour coding: Achievement % — 🔴 <50% · 🟡 50–84% · 🟢 85–99% · ✅ ≥100%. "
        "Day Surgery % — 🔴 <50% · 🟡 50–75% · 🟢 >75%. "
        "Cancellation % — 🟢 <5% · 🟡 5–2% · 🔴 >2%."
    )

    # ── helpers ──────────────────────────────────────────────────────────────
    def _si_pct(num, den):
        """Safe percentage, returns float or NaN."""
        try:
            if den and den > 0:
    ...
    def _si_fmt_pct(v, suffix="%"):
        if v is None or (isinstance(v, float) and np.isnan(v)):
            return "ND"
        return f"{v:.1f}{suffix}"

    def _si_arrow(this_week, last_week):
        """Return ▲/▼/— and colour based on change."""
        try:
    ...
    def _colour_achiev(pct):
        """Background colour for achievement %."""
        try:
            v = float(str(pct).replace("%", "").replace("ND", "nan"))
            if np.isnan(v):
                return ""
            if v >= 100:
                return "background-color:#d4edda;color:#155724;"
    ...
    def _colour_daysurg(pct):
        """Background colour for % day surgery."""
        try:
            v = float(str(pct).replace("%", "").replace("ND", "nan"))
            if np.isnan(v):
                return ""
    ...
    def _colour_cancel(pct):
        """Background colour for % cancellation (lower = better)."""
        try:
            v = float(str(pct).replace("%", "").replace("ND", "nan"))
            if np.isnan(v):
                return ""
    ...
    # ── data prep ─────────────────────────────────────────────────────────────
    # Specialty-level data (Available rows only) — used for most KPIs
    si_df1 = avail1.copy()
    si_df2 = avail2.copy() if comparing else None

    # Score-level data — used for Emergency Surgery Volume (Reconciled)
    # Apply directorate filter to match sidebar selection
    def _filter_score(sdf):
        if sdf is None or sdf.empty:
            return pd.DataFrame(columns=SCORE_COL_NAMES)
        mask = pd.Series(True, index=sdf.index)
        if sel_dir:
            mask = mask & sdf["Directorate"].isin(sel_dir)
        return sdf[mask]

    si_score1 = _filter_score(score1_raw)
    si_score2 = _filter_score(score2_raw) if score2_raw is not None else None

    # ──────────────────────────────────────────────────────────────────────────
    # KPI SOURCE MAPPING (per specification)
    # ──────────────────────────────────────────────────────────────────────────
    # المختبرات المنجزة  (Total surgeries performed this week)
    #   → "Total of surgeries performed during this week"  ← Specialty Level Data
    # عدد العمليات الطارئة  (Emergency Surgery Volume)
    #   → "Emergency Surgery Volume (Reconciled)"          ← Score sheet
    # عدد المرضى الجدد المضافين  (New patients added)
    #   → "Volume of new patients added to the list"       ← Specialty Level Data  (ND if no data, never 0)
    # مجموع قوائم الانتظار  (Total waiting list)
    #   → WL_Unbooked36 + WL_Booked36                     ← Specialty Level Data
    # المرضى المنتظرين أكثر من 36 يوم  (WL >36 days / unbooked)
    #   → WL_Unbooked36                                    ← Specialty Level Data
    # المرضى المجدولين خلال 36 يوم  (WL ≤36 days / booked)
    #   → WL_Booked36                                      ← Specialty Level Data
    # ──────────────────────────────────────────────────────────────────────────

    def _agg_spec_by_dir(df):
        """
        Aggregate specialty-level KPIs per Directorate.
        WL_New: sum only numeric values; if all are NaN/ND treat as ND.
        Total_Surg: sum of 'Total of surgeries performed during this week'.
        WL_Booked36: sum of 'Volume of patients with booked surgeries within 36 days'.
        WL_Unbooked36: sum of 'Volume of patients without booked surgeries within 36 days'.
        """
        return df.groupby("Directorate").agg(
            Total_Surg      = ("Total_Surg",    "sum"),
            OnDay_Surg      = ("OnDay_Surg",    "sum"),
            Elective_Surg   = ("Elective_Surg", "sum"),
            WL_New          = ("WL_New",        "sum"),   # numeric only; ND rows become NaN
            WL_Booked36     = ("WL_Booked36",   "sum"),
            WL_Unbooked36   = ("WL_Unbooked36", "sum"),
            WL_NonSched     = ("WL_NonSched",   "sum"),
        ).reset_index()

    def _agg_score_by_dir(sdf):
        """
        Aggregate score-level KPIs per Directorate.
        Uses 'Emergency Surgery Volume (Reconciled)' column from Score sheet.
        """
        if sdf is None or sdf.empty:
            return pd.DataFrame(columns=["Directorate", "Emergency_Reconciled"])
        col = SCORE_EMERGENCY_COL
        if col not in sdf.columns:
            return pd.DataFrame(columns=["Directorate", "Emergency_Reconciled"])
        return (
            sdf.groupby("Directorate")[col]
            .sum()
            .reset_index()
            .rename(columns={col: "Emergency_Reconciled"})
        )

    spec_agg1  = _agg_spec_by_dir(si_df1)
    score_agg1 = _agg_score_by_dir(si_score1)

    spec_agg2  = _agg_spec_by_dir(si_df2)     if si_df2   is not None else None
    score_agg2 = _agg_score_by_dir(si_score2) if si_score2 is not None else None

    # Merge specialty + score for this week
    agg1 = spec_agg1.merge(score_agg1, on="Directorate", how="left")
    agg1["Emergency_Reconciled"] = agg1["Emergency_Reconciled"].fillna(0)
    # Total WL = WL_Unbooked36 + WL_Booked36
    agg1["WL_Total"] = agg1["WL_Unbooked36"] + agg1["WL_Booked36"]

    if spec_agg2 is not None and score_agg2 is not None:
        agg2 = spec_agg2.merge(score_agg2, on="Directorate", how="left")
        agg2["Emergency_Reconciled"] = agg2["Emergency_Reconciled"].fillna(0)
        agg2["WL_Total"] = agg2["WL_Unbooked36"] + agg2["WL_Booked36"]
    else:
        agg2 = None

    # Merge this-week vs last-week for comparison
    if agg2 is not None:
        merged = agg1.merge(agg2, on="Directorate", how="outer", suffixes=("_w", "_p"))
    else:
        merged = agg1.copy()
        for c in ["Total_Surg", "OnDay_Surg", "Elective_Surg", "WL_New",
                  "WL_Booked36", "WL_Unbooked36", "WL_NonSched",
                  "Emergency_Reconciled", "WL_Total"]:
            merged[f"{c}_w"] = merged[c]
            merged[f"{c}_p"] = np.nan

    directorates_si = sorted(merged["Directorate"].dropna().unique().tolist())

    # ── PAGE 1 — MAIN INDICATOR TABLE ─────────────────────────────────────────
    st.markdown('<div class="exec-section">Page 1 · Achievement & Waiting Indicator by Cluster</div>', unsafe_allow_html=True)

    # ── Kingdom-level summary metrics ──────────────────────────────────────────
    # Total surgeries performed this week  ← Specialty Level Data
    k_total     = int(agg1["Total_Surg"].sum())
    k_onday     = int(agg1["OnDay_Surg"].sum())
    k_elective  = int(agg1["Elective_Surg"].sum())
    # Emergency Surgery Volume (Reconciled)  ← Score sheet
    k_emergency = int(agg1["Emergency_Reconciled"].sum())
    # New patients added  ← Specialty Level Data (ND if no numeric data)
    k_new_raw   = agg1["WL_New"].sum()
    k_new       = int(k_new_raw) if not np.isnan(k_new_raw) else None
    # WL Booked ≤36 days  ← Specialty Level Data
    k_bk36      = int(agg1["WL_Booked36"].sum())
    # WL Unbooked >36 days  ← Specialty Level Data
    k_ubk36     = int(agg1["WL_Unbooked36"].sum())
    # Total WL = Unbooked + Booked  ← Specialty Level Data
    k_wl        = int(agg1["WL_Total"].sum())
    k_nonsched  = int(agg1["WL_NonSched"].sum())

    k_daysurg   = _si_pct(k_onday, k_total)

    kc1, kc2, kc3, kc4, kc5, kc6, kc7, kc8 = st.columns(8)
    kc1.metric("المختبرات المنجزة / Total Surg", f"{k_total:,}")
    kc2.metric("عمليات طارئة / Emergency", f"{k_emergency:,}")
    kc3.metric("مرضى جدد / New Patients",
               fmt(k_new) if k_new is not None else "ND")
    kc4.metric("مجموع قوائم الانتظار / Total WL", f"{k_wl:,}")
    kc5.metric("منتظرون >36 يوم / WL >36 Days", f"{k_ubk36:,}")
    kc6.metric("مجدولون ≤36 يوم / WL ≤36 Days", f"{k_bk36:,}")
    kc7.metric("% Day Surgery", _si_fmt_pct(k_daysurg))
    kc8.metric("% Scheduled ≤36 Days", _si_fmt_pct(_si_pct(k_bk36, k_wl)))
    ...
    for _, row in merged.iterrows():
        ...
        # ── This-week values ──────────────────────────────────────────────────
        # المختبرات المنجزة — Total surgeries performed this week  ← Specialty Level Data
        ts_w  = row.get("Total_Surg_w",          row.get("Total_Surg",          0)) or 0
        od_w  = row.get("OnDay_Surg_w",           row.get("OnDay_Surg",           0)) or 0
        # عدد العمليات الطارئة — Emergency Surgery Volume (Reconciled)  ← Score sheet
        em_w  = row.get("Emergency_Reconciled_w", row.get("Emergency_Reconciled",  0)) or 0
        # عدد المرضى الجدد المضافين — New patients added  ← Specialty Level Data
        nw_raw = row.get("WL_New_w", row.get("WL_New", np.nan))
        nw_w  = None if (nw_raw is None or (isinstance(nw_raw, float) and np.isnan(nw_raw))) else int(nw_raw)
        # مجموع قوائم الانتظار = WL_Unbooked36 + WL_Booked36  ← Specialty Level Data
        bk_w  = row.get("WL_Booked36_w",          row.get("WL_Booked36",          0)) or 0
        ub_w  = row.get("WL_Unbooked36_w",         row.get("WL_Unbooked36",         0)) or 0
        wl_w  = row.get("WL_Total_w",              row.get("WL_Total",              0)) or 0
        ns_w  = row.get("WL_NonSched_w",           row.get("WL_NonSched",           0)) or 0

        # ── Last-week values (for change) ─────────────────────────────────────
        ts_p  = row.get("Total_Surg_p",    np.nan)
        ub_p  = row.get("WL_Unbooked36_p", np.nan)

        daysurg_pct = _si_pct(od_w, ts_w)

        # % change in total surgeries
        if not (isinstance(ts_p, float) and np.isnan(ts_p)) and ts_p and ts_p > 0:
            ts_chg = round((ts_w - ts_p) / ts_p * 100, 1)
        else:
            ts_chg = float("nan")

        # % change in WL >36 days
        if not (isinstance(ub_p, float) and np.isnan(ub_p)) and ub_p and ub_p > 0:
            ub_chg = round((ub_w - ub_p) / ub_p * 100, 1)
        else:
            ub_chg = float("nan")

        # New patients: display ND if no numeric data (never show 0)
        nw_display = fmt(nw_w) if nw_w is not None else "ND"

        table_rows.append({
            "Cluster / Directorate":                        d,
            "المختبرات المنجزة / Total Surg":                  int(ts_w),
            "عمليات طارئة / Emergency Surg":                  int(em_w),
            "مرضى جدد / New Patients":                        nw_display,
            "مجموع قوائم الانتظار / Total WL":                int(wl_w),
            "منتظرون >36 يوم / WL >36 Days":                  int(ub_w),
            "مجدولون ≤36 يوم / WL ≤36 Days":                  int(bk_w),
            "% Day Surgery":                                _si_fmt_pct(daysurg_pct),
            "% Chg Total Surg":                             _si_fmt_pct(ts_chg),
            "% Chg WL >36":                                 _si_fmt_pct(ub_chg),
            "% Scheduled ≤36 Days":                         _si_fmt_pct(_si_pct(bk_w, wl_w)),
            "Non-Scheduled":                                int(ns_w),
        })

    # Grand total row
    k_new_display = fmt(k_new) if k_new is not None else "ND"
    table_rows.append({
        "Cluster / Directorate":                        "GRAND TOTAL",
        "المختبرات المنجزة / Total Surg":                  k_total,
        "عمليات طارئة / Emergency Surg":                  k_emergency,
        "مرضى جدد / New Patients":                        k_new_display,
        "مجموع قوائم الانتظار / Total WL":                k_wl,
        "منتظرون >36 يوم / WL >36 Days":                  k_ubk36,
        "مجدولون ≤36 يوم / WL ≤36 Days":                  k_bk36,
        "% Day Surgery":                                _si_fmt_pct(k_daysurg),
        "% Chg Total Surg":                             "—",
        "% Chg WL >36":                                 "—",
        "% Scheduled ≤36 Days":                         _si_fmt_pct(_si_pct(k_bk36, k_wl)),
        "Non-Scheduled":                                k_nonsched,
    })

    si_table = pd.DataFrame(table_rows)

    # Style the table
    def _style_si_table(df_s):
        styles = pd.DataFrame("", index=df_s.index, columns=df_s.columns)
        last_idx = df_s.index[-1]
        styles.loc[last_idx, :] = "background-color:#1F4E79;color:white;font-weight:700;"
        for idx in df_s.index[:-1]:
            styles.loc[idx, "% Day Surgery"]        = _colour_daysurg(df_s.loc[idx, "% Day Surgery"])
            styles.loc[idx, "% Scheduled ≤36 Days"] = _colour_achiev(df_s.loc[idx, "% Scheduled ≤36 Days"])
        return styles

    styled_si = si_table.style.apply(_style_si_table, axis=None)
    st.dataframe(styled_si, use_container_width=True, height=640)

    # Download button
    csv_si = si_table.to_csv(index=False).encode("utf-8")
    st.download_button(
        "Download Scheduling Indicator Table (CSV)",
        data=csv_si,
        file_name="scheduling_indicator.csv",
        mime="text/csv",
    )

    st.markdown("---")
    st.caption(
        "**Column notes (KPI sources):** "
        "المختبرات المنجزة = Total of surgeries performed this week ← Specialty Level Data · "
        "عمليات طارئة = Emergency Surgery Volume (Reconciled) ← Score sheet · "
        "مرضى جدد = Volume of new patients added (ND if no data, never 0) ← Specialty Level Data · "
        "مجموع قوائم الانتظار = WL_Unbooked36 + WL_Booked36 ← Specialty Level Data · "
        "منتظرون >36 يوم = WL_Unbooked36 ← Specialty Level Data · "
        "مجدولون ≤36 يوم = WL_Booked36 ← Specialty Level Data."
    )

    # ── PAGE 2 — TOP SPECIALTIES BY WL ────────────────────────────────────────
    st.markdown('<div class="exec-section-blue">Page 2 · Top Specialties by Waiting List Volume</div>', unsafe_allow_html=True)
    st.caption(
        "Shows the 14 specialties with highest WL volume. "
        "Upload **File 2** (last week) to see week-over-week arrows."
    )

    # Specialty canonical mapping (same as other tabs)
    SI_SPEC_MAP = {
        "General Surgery":              ["General Surgery"],
        "ENT Surgery - Otolaryngology": ["ENT Surgery - Otolaryngology", "ENT Surgery", "Otolaryngology"],
        "Ophthalmology":                ["Ophthalmology", "Ophthlamology", "Opthalmology"],
    ...
    def _si_spec_wl(df, canonical):
        variants = SI_SPEC_MAP.get(canonical, [canonical])
        return df[df["Specialty"].isin(variants)]["WL_Total"].sum()

    def _si_spec_dir_wl(df, canonical):
        """Return WL by Directorate for a given specialty."""
        variants = SI_SPEC_MAP.get(canonical, [canonical])
        sub = df[df["Specialty"].isin(variants)]
        return sub.groupby("Directorate")["WL_Total"].sum().reset_index(name="WL_Total")

    # Build specialty WL table
    spec_wl_rows = []
    for spec in SI_SPEC_MAP.keys():
        wl_this = _si_spec_wl(si_df1, spec)
        wl_last = _si_spec_wl(si_df2, spec) if si_df2 is not None else None
        arrow, arrow_col = _si_arrow(wl_this, wl_last) if wl_last is not None else ("—", GRAY)
        spec_wl_rows.append({
            "Specialty":                    spec,
            "WL Last Week":                 int(wl_last) if wl_last is not None else "—",
            "WL This Week":                 int(wl_this),
            "Change":                       arrow,
        })

    spec_wl_df = pd.DataFrame(spec_wl_rows)
    spec_wl_df = spec_wl_df.sort_values("WL This Week", ascending=False).reset_index(drop=True)
    spec_wl_df.index = spec_wl_df.index + 1  # 1-based row numbers

    # Style arrows
    def _style_spec_wl(df_s):
        styles = pd.DataFrame("", index=df_s.index, columns=df_s.columns)
        for idx in df_s.index:
            arrow = df_s.loc[idx, "Change"]
            if arrow == "▲":
                styles.loc[idx, "Change"] = f"color:{CORAL};font-weight:700;"
                styles.loc[idx, "WL This Week"] = "font-weight:700;"
            elif arrow == "▼":
                styles.loc[idx, "Change"] = f"color:{TEAL};font-weight:700;"
                styles.loc[idx, "WL This Week"] = "font-weight:700;"
        return styles

    styled_spec_wl = spec_wl_df.style.apply(_style_spec_wl, axis=None)

    col_tbl, col_chart = st.columns([1, 1])

    with col_tbl:
        st.subheader("Top 14 Specialties — WL Volume")
        st.dataframe(styled_spec_wl, use_container_width=True, height=520)

    with col_chart:
        st.subheader("WL Volume by Specialty")
        chart_df = spec_wl_df.copy()
        chart_df["WL This Week"] = pd.to_numeric(chart_df["WL This Week"], errors="coerce")
        chart_df = chart_df.dropna(subset=["WL This Week"]).sort_values("WL This Week")
        fig_spec_wl = px.bar(
            chart_df,
            x="WL This Week", y="Specialty", orientation="h",
            color="WL This Week",
            color_continuous_scale=[[0, "#E1F5EE"], [0.5, AMBER], [1, CORAL]],
            text="WL This Week", height=520,
            labels={"WL This Week": "WL Patients", "Specialty": ""},
        )
        fig_spec_wl.update_coloraxes(showscale=False)
        fig_spec_wl.update_traces(texttemplate="%{text:,.0f}", textposition="outside")
        fig_spec_wl.update_layout(make_layout({"margin": dict(l=0, r=80, t=10, b=10)}))
        fig_spec_wl.update_xaxes(gridcolor="#f0f0f0")
        st.plotly_chart(fig_spec_wl, use_container_width=True)

    # ── Top 5 clusters per specialty ──────────────────────────────────────────
    st.markdown('<div class="exec-section-amber">Top 5 Clusters per Specialty</div>', unsafe_allow_html=True)
    st.caption("Select a specialty to see the top 5 clusters with highest WL volume.")

    sel_si_spec = st.selectbox(
        "Select Specialty",
        list(SI_SPEC_MAP.keys()),
        key="si_spec_sel"
    )

    dir_wl_this = _si_spec_dir_wl(si_df1, sel_si_spec).sort_values("WL_Total", ascending=False).head(5)
    dir_wl_last = _si_spec_dir_wl(si_df2, sel_si_spec) if si_df2 is not None else None

    if len(dir_wl_this) > 0:
        col_top5_l, col_top5_r = st.columns([1, 1])

        with col_top5_l:
            st.subheader(f"Top 5 Clusters — {sel_si_spec}")
            top5_rows = []
            for _, r in dir_wl_this.iterrows():
                d = r["Directorate"]
                wl_t = int(r["WL_Total"])
                wl_l = None
                if dir_wl_last is not None:
                    match = dir_wl_last[dir_wl_last["Directorate"] == d]
                    if len(match) > 0:
                        wl_l = int(match["WL_Total"].iloc[0])
                arrow, _ = _si_arrow(wl_t, wl_l) if wl_l is not None else ("—", GRAY)
                top5_rows.append({
                    "Cluster": d,
                    "WL Last Week": wl_l if wl_l is not None else "—",
                    "WL This Week": wl_t,
                    "Trend": arrow,
                })
            top5_df = pd.DataFrame(top5_rows)
            top5_df.index = top5_df.index + 1
            st.dataframe(top5_df, use_container_width=True, height=240)

        with col_top5_r:
            st.subheader(f"WL by Cluster — {sel_si_spec}")
            fig_top5 = px.bar(
                dir_wl_this.sort_values("WL_Total"),
                x="WL_Total", y="Directorate", orientation="h",
                color="WL_Total",
                color_continuous_scale=[[0, "#E1F5EE"], [1, TEAL]],
                text="WL_Total", height=300,
                labels={"WL_Total": "WL Patients", "Directorate": ""},
            )
            fig_top5.update_coloraxes(showscale=False)
            fig_top5.update_traces(texttemplate="%{text:,.0f}", textposition="outside")
            fig_top5.update_layout(make_layout({"margin": dict(l=0, r=80, t=10, b=10)}))
            fig_top5.update_xaxes(gridcolor="#f0f0f0")
            st.plotly_chart(fig_top5, use_container_width=True)
    else:
        st.info(f"No data found for {sel_si_spec}.")

    # ── All specialties × all clusters heatmap ────────────────────────────────
    st.markdown('<div class="exec-section-coral">All Specialties × All Clusters — WL Heatmap</div>', unsafe_allow_html=True)

    # Build pivot: rows = Directorate, cols = Specialty
    reverse_si = {}
    for canonical, variants in SI_SPEC_MAP.items():
        for v in variants:
            reverse_si[v] = canonical

    si_df1_c = si_df1.copy()
    si_df1_c["Spec_Canon"] = si_df1_c["Specialty"].map(reverse_si).fillna(si_df1_c["Specialty"])
    si_pivot = (
        si_df1_c.groupby(["Directorate", "Spec_Canon"])["WL_Total"]
        .sum()
        .unstack(fill_value=0)
    )
    # Keep only the 14 canonical specs in order
    si_pivot = si_pivot.reindex(columns=[s for s in SI_SPEC_MAP.keys() if s in si_pivot.columns], fill_value=0)

    if len(si_pivot) > 0:
        fig_heat = px.imshow(
            si_pivot,
            color_continuous_scale=[[0, "#f7fbff"], [0.3, "#c6dbef"], [0.7, AMBER], [1, CORAL]],
            aspect="auto",
            text_auto=True,
            height=max(400, len(si_pivot) * 28),
            labels={"x": "Specialty", "y": "Cluster", "color": "WL Patients"},
        )
